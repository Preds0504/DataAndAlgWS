...
public class AVLTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
    ...
    private void rebalance(Position<Entry<K, V>> p) {
    	//trach old height of the node p
        int oldHeight = 0;
        int newHeight = 0;
        ...
        do {
        	...
        	if (!isBalanced(x)) {
        		...
        		//Fidn the grandchild with the taller height
        		Position<Entry<K, V>> grandchild = tallerChild(child);
        		//Perfoirm trinode restructuring at the grandchild
        		// save the root of the restructured suibtree as x
