...
public class AdjacencyListGraph<V, E> extends EdgeListGraph<V, E> {
    ...
    public Iterable<Edge<E>> incomingEdges(Vertex<V> vertex) {
    	ALVertex temp = validate(vertex);
        return temp.getIncoming();
    }
    ...
    public Edge<E> insertEdge(Vertex<V> vertex1, Vertex<V> vertex2, E edgeData) {
        ALVertex v1 = validate(vertex1);
        ALVertex v2 = validate(vertex2);
        ALEdge newEdge = (ALEdge) super.insertEdge(v1, v2, edgeData);
        Position<Edge<E>> outgoingPos = v1.getOutgoing().addLast(newEdge);
        Position<Edge<E>> incomingPos = v2.getIncoming().addLast(newEdge);
        newEdge.setOutgoingListPosition(outgoingPos);
        newEdge.setIncomingListPosition(incomingPos);
